...
(11, 'Alexander', 'Thomas', '1010 Willow Rd', 'St. Johns', 'NL', 'A1A0A1'),
...
INSERT INTO [dbo].[loans] (loan_id, customer_id, loan_amount, interest_rate, loan_term) VALUES
...
INSERT INTO [dbo].[loan_payments] (payment_id, loan_id, payment_date, payment_amount) VALUES
...
select concat(c.first_name,' ',c.last_name),a.balance from [dbo].[accounts] a
join [dbo].[customers] c on a.customer_id = c.customer_id;

select concat(c.first_name,' ',c.last_name),l.loan_amount from [dbo].[loans] l 
join [dbo].[customers] c on l.customer_id = c.customer_id;


select concat(c.first_name,' ',c.last_name),t.transaction_date from [dbo].[transactions] t 
join [dbo].[accounts] a on t.account_id = a.account_id
join [dbo].[customers] c on a.customer_id = c.customer_id
where t.transaction_date LIKE '2024-03-%';
...
select c.customer_id,c.first_name,c.last_name,sum(a.balance) from [dbo].[accounts] a 
join [dbo].[customers] c on a.customer_id = c.customer_id
group by c.customer_id,c.first_name,c.last_name;


select loan_term,avg(loan_amount) from [dbo].[loans]
group by loan_term


select sum(loan_amount) as total_loan, sum(loan_amount * (interest_rate/100)) as total_interest
 from [dbo].[loans]

select top 1 transaction_type,count(transaction_id) as transactioncount from [dbo].[transactions]
group by transaction_type
order by transactioncount desc;


select a.account_type, t.transaction_type, count(t.transaction_id) as count_transactions,SUM(t.transaction_amount) AS total_transaction_amount from [dbo].[transactions] t 
join [dbo].[accounts] a on t.account_id = a.account_id
...
create view active_loans_1000 as 
